...
description: "Dark-themed SVG process flow diagrams as HTML (workflows, approvals, automation)."
version: 1.1.0
author: Cocoon AI (hello@cocoon-ai.com), ported for Cursor
license: MIT
dependencies: []
platforms: [linux, macos, windows]
metadata:
  hermes:
    tags: [process-flow, workflow, diagrams, SVG, HTML, visualization, automation]
...
Based on [Cocoon AI's process-flow-diagram-generator](https://github.com/Cocoon-AI/process-flow-diagram-generator) (MIT).

## Workflow

1. User describes their process (steps, actors, decisions, integrations)
2. Generate the HTML file following the design system below
3. Save with the Write tool to a `.html` file (e.g. `./my-workflow-flow.html`)
4. User opens in any browser — export toolbar supports Copy / PNG / PDF

### Output Location

Save diagrams to a user-specified path, or default to the current working directory:
```
./[process-name]-flow.html
```
...
```bash
# macOS
open ./my-workflow-flow.html
# Linux
xdg-open ./my-workflow-flow.html
```
...
Skip when: the relationships are non-sequential (system component graphs, infrastructure topologies). For those, use the `architecture-diagram` skill (`.cursor/skills/architecture-diagram/SKILL.md`).
...
Load the full HTML template for structure, CSS, export toolbar, and SVG patterns:

```
Read: .cursor/skills/process-flow-diagram/templates/template.html
```
...
**IMPORTANT: Preview the diagram before delivering.**

1. Save the HTML file and open it in a browser (or use browser MCP tools if available)
2. Check for common issues below; fix and re-open if needed
